...
def run(question: str, max_steps: int = 10, verbose: bool = True, history: list = None) -> Generator[dict, None, None]:
    ...
    新增 history 参数：传入已有对话历史（messages 列表），实现多轮对话续接。
              history 列表会被原地修改，迭代结束后调用方可通过 history 获取完整对话消息。
...
def run_and_print(question: str, max_steps: int = 10, history: list = None) -> list:
    """
    新增 history 参数，返回更新后的历史消息列表，供下一轮使用。
    """
    if history is None:
        history = []
    print(f"\n{'='*60}")
