...
DATA = ROOT / "data"/"bq_corpus"
BERT_PATH = ROOT / "models" / "google-bert" / "bert-base-chinese"
...
def collect_biencoder_preds(model, loader, raw_rows, device, threshold):
    ...
    for batch in loader:
        ...
        em_a, em_b = model(batch_a, batch_b)
        sims= F.cosine_similarity(em_a, em_b, dim=-1).cpu().numpy()
        labels = batch["label"].tolist()

        for sim, pred in zip(sims, labels):
            row = raw_rows[idx]
            results.append({
...
【1】数据层面
  ...
  │
  ├─ 数据增强（正样本扩充）
  │    bq_corpus 正样本只有 10K 条（31%），TripletLoss 三元组因此受限。
...
def main():
    ...
    # ── 加载原始数据和 DataLoader ─────────────────────────────────────────
    data_path = DATA / f"{args.split}.jsonl"
