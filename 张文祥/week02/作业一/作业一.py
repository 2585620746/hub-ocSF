...
规律：x是一个5维向量，第一维向量最大就属于第一类，第二维向量最大就属于第二类....（5分类）
...
class TorchModel(nn.Module):
    def __init__(self, input_size, num_classes):
        ...
        self.loss = nn.functional.cross_entropy  # 交叉熵损失，适合多分类
...
def predict(model_path, input_vec):
    ...
    for vec, pred in zip(input_vec, pred_classes):
        pred = pred + 1
        max_dim = np.argmax(vec) + 1
        print("输入：%s, 最大维度：%d, 预测类别：%d" % (vec, max_dim, pred))


if __name__ == "__main__":
    main()
    #test_vec = [[7,9,6,5,2],
                #[0.92963533,0.5524256,0.95758807,0.85520434,0.84890681],
